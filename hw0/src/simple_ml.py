...
def softmax_loss(Z, y):
    ...
    for i in range (batch_size):
      cur_loss = np.log(np.sum (np.exp (Z[i]))) - Z[i][y[i]]
      loss_list.append (cur_loss)
    return np.average (np.array (loss_list))
    ### END YOUR CODE
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for l in range (0, num_examples, batch):
      ...
      curX = X[l:r][:]
      XTheta = np.matmul (curX, theta)
      XTheta = np.exp (XTheta)
      row_sums = XTheta.sum (axis=1)
      Z = XTheta / row_sums[:, np.newaxis]
      I_y = np.zeros ((r - l, num_classes))
      for i in range (l, r):
        I_y[i - l][y[i]] = 1
      theta -= lr / (r - l) * np.matmul (curX.T, Z - I_y)
    ### END YOUR CODE
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for l in range (0, num_examples, batch):
      r = min (num_examples, l + batch)
      curX = X[l:r][:]
      Z1 = relu (np.matmul (curX, W1))
      G2 = np.exp (np.matmul (Z1, W2))
      row_sums = G2.sum (axis=1)
      G2 /= row_sums[:, np.newaxis]
      Iy = np.zeros ((r - l, num_classes))
      Iy[np.arange (r - l), y[l:r]] = 1
      G2 -= Iy
      G1 = np.multiply ((Z1 > 0), np.matmul (G2, W2.T))
      W1 -= lr / (r - l) * np.matmul (curX.T, G1)
      W2 -= lr / (r - l) * np.matmul (Z1.T, G2)
    ### END YOUR CODE
