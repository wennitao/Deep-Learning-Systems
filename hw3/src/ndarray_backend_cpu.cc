...
void Compact(const AlignedArray& a, AlignedArray* out, std::vector<uint32_t> shape,
  ...
  int dim = shape.size() ;
  std::vector<int> idx ;
  for (int i = 0; i < dim; i ++) idx.push_back (0) ;
  idx[dim - 1] = -1 ;
  int cnt = 0 ;
  while (1) {
    bool finished = true ;
    for (int i = 0; i < dim; i ++)
      if (idx[i] != shape[i] - 1) finished = false ;
    if (finished) break ;
    idx[dim - 1] ++ ;
    int cur = dim - 1 ;
    while (cur && idx[cur] >= shape[cur]) {
      idx[cur] = 0; idx[-- cur] ++ ;
    }
    int real_idx = offset ;
    for (int i = 0; i < dim; i ++)
      real_idx += idx[i] * strides[i] ;
    out -> ptr[cnt ++] = a.ptr[real_idx] ;
  }
  /// END YOUR SOLUTION
}
  ...
  for (int i = 0; i < m / TILE; i ++)
    for (int j = 0; j < p / TILE; j ++) {
      ...
      for (int k = 0; k < n / TILE; k ++)
        AlignedDot (a.ptr + i * n / TILE * TILE * TILE + k * TILE * TILE, b.ptr + k * p / TILE * TILE * TILE + j * TILE * TILE, c) ;
      for (int k = 0; k < TILE * TILE; k ++)
        out -> ptr[i * p / TILE * TILE * TILE + j * TILE * TILE + k] = c[k] ;
    }
  /// END YOUR SOLUTION
}
...
void ReduceMax(const AlignedArray& a, AlignedArray* out, size_t reduce_size) {
  ...
  /// BEGIN YOUR SOLUTION
  for (int i = 0; (i + 1) * reduce_size <= a.size; i ++) {
