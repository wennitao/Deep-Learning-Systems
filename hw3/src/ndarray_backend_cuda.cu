...
__device__ size_t IndexToOffset (size_t index, CudaVec shape, CudaVec strides, size_t offset) {
  int dim = shape.size ;
  size_t real_idx = offset ;
  for (int i = dim - 1; i >= 0; i --) {
    int idx = index % shape.data[i] ;
    index /= shape.data[i] ;
    real_idx += idx * strides.data[i] ;
  }
  return real_idx ;
}
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  /// BEGIN YOUR SOLUTION
  if (gid < size) out[gid] = a[IndexToOffset (gid, shape, strides, offset)] ;
  /// END YOUR SOLUTION
}
...
__global__ void EwiseSetitemKernel (const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
                              CudaVec strides, size_t offset) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  // printf("gid: %d, idx: %d\n", gid, IndexToOffset (gid, shape, strides, offset)) ;
  if (gid < size) out[IndexToOffset (gid, shape, strides, offset)] = a[gid] ;                           
}
...
__global__ void ScalarSetitemKernel (const scalar_t val, scalar_t* out, size_t size, CudaVec shape,
                              CudaVec strides, size_t offset) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) out[IndexToOffset (gid, shape, strides, offset)] = val ;                           
}
...
__global__ void MatmulKernel (const scalar_t* a, const scalar_t* b, scalar_t* out, uint32_t M, uint32_t N,
    ...
    if (row < M && col < P) {
      for (int i = 0; i < N; i ++)
          CValue += a[row * N + i] * b[i * P + col] ;
      out[row * P + col] = CValue ;
    }
}
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  dim3 dimGrid ((P - 1) / dimBlock.x + 1, (M - 1) / dimBlock.y + 1) ;
