...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for i in reverse_topo_order:
      ...
      if i.op == None:
        continue
      # print (i.op, isinstance (i.grad, tuple))
      # if isinstance (i.op, needle.ops.MakeTensorTuple) and not isinstance (i.grad, tuple):
      #   v_i = (v_i, )
      # print (i.op, i, v_i, i.inputs)
      grad = i.op.gradient (v_i, i)
      for idx in range (len (i.inputs)):
        k = i.inputs[idx]
        v_ki = grad[idx]
        # print ('gradient type: ', v_ki.dtype)
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    visited = set()
    topo_order = []
    topo_sort_dfs (beg, visited, topo_order)
    return topo_order
    ### END YOUR SOLUTION
...
def topo_sort_dfs(node, visited, topo_order):
    """Post-order DFS"""
    ### BEGIN YOUR SOLUTION
    for pre in node.inputs:
      if pre not in visited:
        # print (pre.op, ' to ', node.op)
        visited.add (pre)
        topo_sort_dfs (pre, visited, topo_order)
