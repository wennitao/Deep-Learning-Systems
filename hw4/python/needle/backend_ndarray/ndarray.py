...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        for x in self.shape:
          cur_shape_size *= x
        if cur_shape_size != new_shape_size:
          raise ValueError
        ...
        new_stride = []
        cur_stride = 1
        new_stride_list = list (new_shape)
        new_stride_list.reverse()
        for x in new_stride_list:
          new_stride.append (cur_stride)
          cur_stride *= x
        new_stride.reverse()
        return NDArray.make (new_shape, strides=tuple (new_stride), device=self.device, handle=self._handle)
        ### END YOUR SOLUTION
    ...
    def permute(self, new_axes):
        ...
        new_stride = []
        for idx in new_axes:
          new_stride.append (self.strides[idx])
        return NDArray.make (tuple(new_shape), strides=tuple (new_stride), device=self.device, handle=self._handle)
        ### END YOUR SOLUTION
    ...
    def broadcast_to(self, new_shape):
        ...
        #   assert self.shape[idx] == 1 or self.shape[idx] == new_shape[idx]
        # print (self.shape)
        # print (new_shape)
        new_stride = []
        cur_stride = 1
        for idx in range (min (len (self.shape), len (new_shape)) - 1, -1, -1):
          if self.shape[idx] == 1 and self.shape[idx] != new_shape[idx]:
            new_stride.append (0)
          else:
            new_stride.append (cur_stride)
          cur_stride *= self.shape[idx]
        new_stride.reverse()
        return NDArray.make (tuple(new_shape), strides=tuple (new_stride), device=self.device, handle=self._handle)
        ### END YOUR SOLUTION
    ...
    def __getitem__(self, idxs):
        ...
        for idx in range (len (idxs)):
          cur_slice = idxs[idx]
          new_shape.append ((cur_slice.stop - cur_slice.start - 1) // cur_slice.step + 1)
          new_stride.append (cur_slice.step * self.strides[idx])
          offset += cur_slice.start * self.strides[idx]
        return NDArray.make (tuple(new_shape), strides=tuple (new_stride), device=self.device, handle=self._handle, offset = offset)
        ### END YOUR SOLUTION
...
def stack (arrays, axis):
    np_arrays = [array.numpy() for array in arrays]
    return NDArray (np.stack (np_arrays, axis))

def split (a, indices, axis):
    # print (a)
    # print (indices, axis)
    splitList = np.split (a.numpy(), indices, axis)
    ndarrayList = [NDArray (ndarray) for ndarray in splitList]
    return ndarrayList
